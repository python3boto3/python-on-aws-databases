...
def configure_dataset_directory(csv_files, dataset_dir):
    # Make a dataset folder for processing of the csv files
    try:
...
def clean_tbl_name(filename):
    ...
    clean_tbl_name = filename.lower().replace(
        " ", "").replace("-", "_").replace(r"/", "_") \
...
def upload_to_db(host, dbname, user, pw, tbl, col_str, file, df, df_columns):

    conn_string = "host=%s dbname=%s user=%s pw=%s" % (host, dbname, user, pw)
    ...
    # create table
    cursor.execute("create table %s (%s);" % (tbl, col_str))
    print('{0} was created successfully'.format(tbl))

    # insert values to table

    # save df to csv
    df.to_csv(file, header=df_columns, index=False, encoding='utf-8')
    ...
    cursor.copy_expert(sql=SQL_STATEMENT % tbl, file=my_file)
    print('file copied to db')

    cursor.execute("grant select on table %s to public" % tbl)
    conn.commit()
    cursor.close()
    print('table {0} imported to db completed'.format(tbl))
